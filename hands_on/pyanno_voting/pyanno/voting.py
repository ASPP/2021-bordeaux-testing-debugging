...
def labels_frequency(annotations, nclasses):
    ...
    """
    a = np.arange(0,nclasses)
    print(a)
    frequency = []
    l = sum(annotations, [])
    print(l)
    sum_freq = len(l) - l.count(-1)
    ...
    return np.array(frequency, dtype = np.double)/ sum_freq
		#labels, frequency = np.unique(annotations,return_counts = True)
